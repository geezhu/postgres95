...
int
Pg_connect(ClientData cData, Tcl_Interp *interp, int argc, const char *argv[]) {
    char *pghost = NULL;
    char *pgtty = NULL;
    char *pgport = NULL;
    char *pgoptions = NULL;
...
int
Pg_disconnect(ClientData cData, Tcl_Interp *interp, int argc, const char *argv[]) {
    PGconn *conn;
    char *connPtrName;
...
int
Pg_exec(ClientData cData, Tcl_Interp *interp, int argc, const char *argv[]) {
    PGconn *conn;
    PGresult *result;
    char *connPtrName;
    ...
    if (result) {
        char tcl_result[20];
        PgSetId(tcl_result, (void *) result);
 ...
 **********************************/
int
Pg_result(ClientData cData, Tcl_Interp *interp, int argc, const char *argv[]) {
    char *resultPtrName;
    PGresult *result;
    char *opt;
    int i;
    ...
    if (strcmp(opt, "-status") == 0) {
        ...
        char tcl_result[20];
        PgSetId(tcl_result, (void *) result->conn);
        Tcl_SetResult(interp, tcl_result, TCL_VOLATILE);
        return TCL_OK;
        ...
        char tcl_result[11];
        sprintf(tcl_result, "%d", PQntuples(result));
        Tcl_SetResult(interp, tcl_result, TCL_VOLATILE);
        return TCL_OK;
...
int
Pg_lo_open(ClientData cData, Tcl_Interp *interp, int argc, const char *argv[]) {
    PGconn *conn;
    char *connPtrName;
    int lobjId;
    int mode;
...
**********************/
int
Pg_lo_close(ClientData cData, Tcl_Interp *interp, int argc, const char *argv[]) {
    PGconn *conn;
    char *connPtrName;
    //max string length of INT32 = 10,and plus \0
    char result[11];
    int fd;
    ...
    conn = (PGconn *) PgGetId(connPtrName);
    fd = atoi(argv[2]);
    sprintf(result, "%d", lo_close(conn, fd));
    Tcl_SetResult(interp, result, TCL_VOLATILE);
    return TCL_OK;
}
...
**********************/
int
Pg_lo_read(ClientData cData, Tcl_Interp *interp, int argc, const char *argv[]) {
    PGconn *conn;
    char *connPtrName;
    int fd;
    int nbytes = 0;
...
***********************************/
int
Pg_lo_write(ClientData cData, Tcl_Interp *interp, int argc, const char *argv[]) {
    PGconn *conn;
    char *connPtrName;
    char *buf;
    int fd;
...
***********************************/
int
Pg_lo_lseek(ClientData cData, Tcl_Interp *interp, int argc, const char *argv[]) {
    PGconn *conn;
    char *connPtrName;
    int fd;
    char *whenceStr;
    ...
    char tcl_result[11];
    sprintf(tcl_result, "%d", lo_lseek(conn, fd, offset, whence));
    Tcl_SetResult(interp, tcl_result, TCL_VOLATILE);
    return TCL_OK;
}
...
***********************************/
int
Pg_lo_creat(ClientData cData, Tcl_Interp *interp, int argc, const char *argv[]) {
    PGconn *conn;
    char *connPtrName;
    char *modeStr;
    char *modeWord;
    ...
    char tcl_result[11];
    sprintf(tcl_result, "%d", lo_creat(conn, mode));
    Tcl_SetResult(interp, tcl_result, TCL_VOLATILE);
    return TCL_OK;
}
...
***********************************/
int
Pg_lo_tell(ClientData cData, Tcl_Interp *interp, int argc, const char *argv[]) {
    PGconn *conn;
    char *connPtrName;
    int fd;
    ...
    fd = atoi(argv[2]);
    char tcl_result[11];
    sprintf(tcl_result, "%d", lo_tell(conn, fd));
    Tcl_SetResult(interp, tcl_result, TCL_VOLATILE);
    return TCL_OK;
...
***********************************/
int
Pg_lo_unlink(ClientData cData, Tcl_Interp *interp, int argc, const char *argv[]) {
    PGconn *conn;
    char *connPtrName;
    int lobjId;
    int retval;
...
int
Pg_lo_import(ClientData cData, Tcl_Interp *interp, int argc, const char *argv[]) {
    PGconn *conn;
    char *connPtrName;
    char *filename;
    Oid lobjId;
...
int
Pg_lo_export(ClientData cData, Tcl_Interp *interp, int argc, const char *argv[]) {
    PGconn *conn;
    char *connPtrName;
    char *filename;
    Oid lobjId;
    ...
    if (retval == -1) {
        char tcl_result[22 + 10 + strlen(filename)];
        sprintf(tcl_result, "Pg_lo_export %d %s failed", lobjId, filename);
        Tcl_SetResult(interp, tcl_result, TCL_VOLATILE);
        return TCL_ERROR;
    }
    return TCL_OK;
