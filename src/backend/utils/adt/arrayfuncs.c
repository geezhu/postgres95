...
_ReadLOArray(char *str,
    ...
    while (str != NULL) {
        ...
        if (!strcmp(word, "-chunk")) {
            ...
            elog(WARN,
                 "usage: <input file> -chunk DEFAULT/<access pattern file> -invert/-native [-noreorg <chunk file>]");
        }
    }
...
array_ref(ArrayType *array,
    ...
    if (!SanityCheckInput(ndim, n, dim, lb, indx)) RETURN_NULL;
    ...
    if (ARR_IS_LO(array)) {
        ...
        if (ARR_IS_CHUNKED(array))
            ...
            if (lo_lseek(fd, offset, SEEK_SET) < 0) RETURN_NULL;
#ifdef LOARRAY
            v = (struct varlena *) LOread(fd, elmlen);
#endif
        }
        if (*isNull) RETURN_NULL;
        if (VARSIZE(v) - 4 < elmlen) RETURN_NULL;
        (void) lo_close(fd);
...
array_clip(ArrayType *array,
    ...
    if (!SanityCheckInput(ndim, n, dim, lb, upperIndx)) RETURN_NULL;

    if (!SanityCheckInput(ndim, n, dim, lb, lowerIndx)) RETURN_NULL;
