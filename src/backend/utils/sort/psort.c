    ...
    while (times-- != 0) {            /* merge one run */
        ...
        if (TotalDummy == 0)
            for (tp = dest->tp_prev; tp != dest; tp = tp->tp_prev) {
                ...
                USEMEM(tuplen);
                TRACEMEM(merge);
                SETTUPLEN(tup, tuplen);GETTUP(tup, tuplen, tp->tp_file);
        ...
        while (tuples != NULL) {
            ...
            if (tuplen == 0);
                ...
                USEMEM(tuplen);
                TRACEMEM(merge);
                SETTUPLEN(tup, tuplen);GETTUP(tup, tuplen, Tape[fromtape].tp_file);
...
endpsort(Relation rdesc, FILE *file) {
    ...
    if (!feof(file))
        while (GETLEN(tuplen, file) && tuplen != 0) {
            tup = ALLOCTUP(tuplen);
            SortMemory += tuplen;
            SETTUPLEN(tup, tuplen);GETTUP(tup, tuplen, file);
