...
# first, get the oid of the rel
    ...
    res [pg_exec
    $conn
    "SELECT oid FROM pg_class where relname = '$rel'"]
    if {
        [pg_result
        $res - numTuples] == "0"
    }
    {
        puts
        stderr
        "update_attnvals: Relation named $rel was not found"
        return
    }
    set
    oid [pg_result
...
# datnames is the list to be result
    ...
    res [pg_exec
    $conn
    "SELECT relname FROM pg_class WHERE relkind = 'r' and relname !~ '^pg_' and relname !~ '^Xinv'"]
    set
    ntups [pg_result
    $res - numTuples]
    for
    ...
    }
    {
        ...
        $i]
        puts
        "updating attnvals stats on table $rel"
        update_attnvals
        $conn $rel
    }
    pg_disconnect $conn
